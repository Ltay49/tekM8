...
export default function EnterDetails() {
    ...
    const checkSaved = async () => {
      ...
      if (saved) {
        try {
          ...
          // Only set values for firstName, surname, title, email, and signature
          setForm(prevForm => ({
            ...prevForm,
            firstName: parsed.firstName || prevForm.firstName,
            surname: parsed.surname || prevForm.surname,
            title: parsed.title || prevForm.title,
            email: parsed.email || prevForm.email,
            signature: parsed.signature || prevForm.signature,
          }));
        } catch (e) {
  ...
  const handleSave = async () => {
    try {
      // Get existing userDetails from AsyncStorage (if any)
      const savedDetailsJSON = await AsyncStorage.getItem('userDetails');
      let updatedDetails = [];
  
      // If there are existing details, parse them and add the new form data
      if (savedDetailsJSON) {
        updatedDetails = JSON.parse(savedDetailsJSON);
      }
  
      // Add the current form data to the array
      updatedDetails.push(form);
  
      // Save the updated array back to AsyncStorage
      await AsyncStorage.setItem('userDetails', JSON.stringify(updatedDetails));
  
      // Log the saved userDetails array with signature (first 30 chars)
      const debugLogDetails = updatedDetails.map((item: any) => ({
        ...item,
        signature: item.signature ? item.signature.slice(0, 30) : '', // Log only first 30 chars of the signature
      }));
      console.log('UserDetails saved as array:', JSON.stringify(debugLogDetails));
      ...
      // Emit event to notify that the details are saved
      eventEmitter.emit('detailsSaved');
    } catch (e) {
      Alert.alert('Error', 'Failed to save details.');
      console.error('Error saving userDetails:', e);
    }
  };
